...
def test_convert_to_joint_trajectory():
    ...
    roboplan_traj.positions = [
        np.array([0.0, 0.0]),
        np.array([1.0, -1.0])
    ]
    roboplan_traj.velocities = [
        np.array([0.0, 0.0]),
        np.array([2.0, -2.0])
    ]
    roboplan_traj.accelerations = [
        np.array([0.0, 0.0]),
        np.array([3.0, -3.0])
    ]
    ...
    for i in range(len(roboplan_traj.positions)):
        assert new_roboplan_traj.positions[i] == pytest.approx(roboplan_traj.positions[i])

    assert len(new_roboplan_traj.velocities) == len(roboplan_traj.velocities)
    for i in range(len(roboplan_traj.velocities)):
        assert new_roboplan_traj.velocities[i] == pytest.approx(roboplan_traj.velocities[i])

    assert len(new_roboplan_traj.accelerations) == len(roboplan_traj.accelerations)
    for i in range(len(roboplan_traj.accelerations)):
        assert new_roboplan_traj.accelerations[i] == pytest.approx(roboplan_traj.accelerations[i])
